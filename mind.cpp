...
std::string Mind::TBAComp(std::string expr) const {
  ...
  trim_string(expr);
  if (0
      //	|| body->Skill(prhash("TBAScript")) == 5000099
...
Mind::~Mind() {
  // if(body && body->Skill(prhash("TBAScript")) >= 5034503 && body->Skill(prhash("TBAScript")) <=
  // 5034507)
  //  fprintf(stderr, CBLU "Disabled(%p): '%d'\n" CNRM, this, body->Skill(prhash("TBAScript")));
  if (type == MIND_REMOTE)
    close_socket(pers);
  type = MIND_MORON;
  Unattach();
...
void Mind::SetTBATrigger(Object* tr, Object* tripper, Object* targ, std::string text) {
  ...
  int stype = body->Skill(prhash("TBAScriptType"));
...
bool Mind::Send(const std::string& mes) {
  if (type == MIND_REMOTE) {
    SendOut(pers, mes);
    ...
    // HELPER TBA Mobs
    if (body && body->Parent() && (body->Skill(prhash("TBAAction")) & 4096) // Helpers
        && ((body->Skill(prhash("TBAAction")) & 2) == 0) // NON-SENTINEL
...
std::string Mind::Tactics(int phase) const {
  if (type == MIND_TBAMOB) {
    ...
    if (body && (body->Skill(prhash("TBAAction")) & 4128) == 0) {
      if (phase == -1) {
...
bool Mind::TBAVarSub(std::string& line) const {
  ...
  while (cur != std::string::npos) {
    ...
    if (0
      ...
      fprintf(
          stderr, CGRN "#%d Debug: '%s'\n" CNRM, body->Skill(prhash("TBAScript")), line.c_str());
    }
    ...
    if (ovars.count(vname) > 0) {
      ...
      while (room && room->Skill(prhash("TBARoom")) == 0)
        room = room->Parent();
      if (room) {
    ...
    while (line[end] == '.') {
      ...
      if (is_obj) {
        if (!strcmp(field.c_str(), "id")) {
          ...
          int vnum = 0;
          if (obj) {
            vnum = obj->Skill(prhash("TBAMOB"));
            if (vnum < 1)
              vnum = obj->Skill(prhash("TBAObject"));
            if (vnum < 1)
              vnum = obj->Skill(prhash("TBARoom"));
            if (vnum > 0)
          ...
          if (obj) {
            if (obj->HasSkill(prhash("Conatiner")))
              ...
              val = "FOUNTAIN";
            else if (obj->HasSkill(prhash("Liquid Container")))
              val = "LIQUID CONTAINER";
            else if (obj->HasSkill(prhash("Ingestible")) <= 0)
              val = "FOOD";
            else if (obj->Value() <= 0)
              val = "TRASH";
            // FIXME: More Types!
          ...
          if (obj) {
            int align = 0;
            align = obj->Skill(prhash("Honor"));
            if (align == 0)
              align = -(obj->Skill(prhash("Dishonor")));
            val = itos(align);
          }
          obj = nullptr;
          is_obj = 0;
          ...
          val = "";
          if (obj)
            val = itos(obj->Skill(prhash("Temporary"))); // FIXME: More Kinds?
          obj = nullptr;
          is_obj = 0;
          ...
          val = "";
          if (obj) {
            if (obj->HasSkill(prhash("Faith"))) {
              val = itos(obj->Skill(prhash("Faith Remaining")));
            } else {
              val = itos(10 - obj->Stun());
            }
          }
          ...
          if (obj) {
            if (obj->HasSkill(prhash("Faith"))) {
              val = itos(obj->Skill(prhash("Faith")) * obj->Skill(prhash("Faith")));
            } else {
              val = "10";
            }
          }
          ...
          val = "";
          if (obj)
            val = itos(obj->Skill(prhash("Hungry"))); // FIXME: Convert
          obj = nullptr;
          is_obj = 0;
        } else if (!strcmp(field.c_str(), "thirst")) {
          val = "";
          if (obj)
            val = itos(obj->Skill(prhash("Thirsty"))); // FIXME: Convert
          obj = nullptr;
          is_obj = 0;
          ...
          val = "";
          if (obj) {
            if (obj->HasSkill(prhash("Spellcasting")) || obj->HasSkill(prhash("Spellcraft"))) {
              val = "magic user";
            } else if (obj->HasSkill(prhash("Perception")) || obj->HasSkill(prhash("Stealth"))) {
              val = "thief";
            } else if (obj->HasSkill(prhash("Faith"))) {
              val = "priest";
            } else {
              val = "warrior";
            }
          ...
          val = "";
          if (obj)
            val = bstr[!(obj->HasSkill(prhash("Invisible")) || obj->HasSkill(prhash("Hidden")))];
          obj = nullptr;
          is_obj = 0;
          ...
          while (obj && obj->Skill(prhash("TBARoom")) == 0)
            obj = obj->Parent();
          ...
          if (obj && vnum != -1 && field[num] == ')') {
            ...
            for (auto item : pos) {
              if (vnum == item->Skill(prhash("TBAObject"))) {
                val = "1";
                break;
              }
            }
          ...
          if (obj) {
            int vnum = obj->Skill(prhash("TBAMOB"));
            if (vnum < 1)
              vnum = obj->Skill(prhash("TBAObject"));
            if (vnum < 1)
              vnum = obj->Skill(prhash("TBARoom"));
...
#define QUOTAERROR2 body->Skill(prhash("TBAScript"))
#define PING_QUOTA()                             \
  {                                              \
    --quota;                                     \
    if (quota < 1) {                             \
...
int Mind::TBARunLine(std::string line) {
  ...
  Object* room = ovars.at("self");
  while (room && room->Skill(prhash("TBARoom")) == 0) {
    if (room->Skill(prhash("Invisible")) > 999)
  ...
  if (!room) { // Not in a room (dup clone, in a popper, etc...).
    ...
    //	body->Skill(prhash("TBAScript")), line.c_str()
    //	);
    return -1;
  }
  // Needs to be alive! MOB & MOB-* (Not -DEATH or -GLOBAL)
  if ((body->Skill(prhash("TBAScriptType")) & 0x103FFDE) > 0x1000000) {
  ...
  int vnum = body->Skill(prhash("TBAScript"));
  if (!TBAVarSub(line)) {
  ...
  if (!strncmp(line.c_str(), "unset ", 6)) {
    ...
    if (lpos != std::string::npos) {
      ...
      if (end1 != std::string::npos) {
        ...
        if (lpos != std::string::npos) {
          ...
          if (coml == 'e') {
            int valnum = body->Skill(prhash("TBAScript"));
            if (!TBAVarSub(val)) {
    ...
    for (auto opt : options) {
      int tnum = opt->Skill(prhash("TBARoom"));
      if (tnum > 0 && tnum == dnum) {
        room = opt;
        break;
      }
    ...
    if (sscanf(line.c_str() + 7, " %s OBJ:%p", var, &con) >= 2 && con != nullptr) {
      ...
      if (con->IsAnimate()) {
        con->Accomplish(body->Skill(prhash("Accomplishment")), "completing a quest");
      }
    ...
    if (sscanf(line.c_str() + 7, " %s OBJ:%p", var, &con) >= 2 && con != nullptr) {
      if (svars.count(var) > 0) {
        ...
        if (con->IsAnimate()) {
          con->Accomplish(body->Skill(prhash("Accomplishment")), "role playing");
        }
        //	fprintf(stderr, CGRN "#%d Debug: Remote %s=%d '%s'\n" CNRM,
        //		body->Skill(prhash("TBAScript")), var, val, line.c_str());
      } else {
        fprintf(
            stderr,
    ...
    if (sscanf(line.c_str() + 11, "%d:%d", &hour, &minute) > 0) {
      ...
      if (world->Skill(prhash("Day Time")) && world->Skill(prhash("Day Length"))) {
        cur = world->Skill(prhash("Day Time"));
        cur *= 24 * 60;
        cur /= world->Skill(prhash("Day Length"));
      }
      if (minute < cur)
        minute += 24 * 60; // Not Time Until Tomorrow!
      if (minute > cur) { // Not Time Yet!
        Suspend((minute - cur) * 1000 * world->Skill(prhash("Day Length")) / 24);
    ...
    if (time > 0) {
      // if(body->Skill(prhash("TBAScript")) >= 5034503 && body->Skill(prhash("TBAScript")) <=
      // 5034507)
      //  fprintf(stderr, CBLU "#%d Suspending for: %d\n" CNRM, body->Skill(prhash("TBAScript")),
      //  time * 1000);
      Suspend(time * 1000);
      return 1;
    } else {
    ...
    if (sscanf(line.c_str() + end, " %d %d", &v1, &v2) != 2) {
      fprintf(
          ...
          line.c_str());
      return -1;
    } else if (ovars["self"]->Skill(prhash("Liquid Source")) && v1 == 0) {
      if (v2 < 0)
        v2 = 1 << 30;
      ovars["self"]->SetSkill(prhash("Liquid Source"), v2 + 1);
    } else if (ovars["self"]->Skill(prhash("Liquid Source")) && v1 == 1) {
      ...
      ovars["self"]->Contents().front()->SetSkill(prhash("Quantity"), v2 + 1);
    } else {
      fprintf(
          stderr,
    ...
    mes[strlen(mes)] = '\n';
    Object* troom = targ;
    while (troom && troom->Skill(prhash("TBARoom")) == 0)
      troom = troom->Parent();
    if (troom && targ)
    ...
    for (auto opt : options) {
      if (opt->Matches(buf2)) {
        if (dam > 0) {
          ...
          //		body->Skill(prhash("TBAScript")), opt->Noun().c_str(), dam
          //		);
        } else if (dam < 0) {
          opt->HealStun(((-dam) + 1) / 2);
          opt->HealPhys(((-dam) + 1) / 2);
          //	  fprintf(stderr, CGRN "#%d Debug: WHeal '%s', %d\n" CNRM,
          //		body->Skill(prhash("TBAScript")), opt->Noun().c_str(), ((-dam)+1)/2
          //		);
        }
      }
    }
    ...
    rnum += 1000000;
    for (auto opt : options) {
      int tbanum = opt->Skill(prhash("TBARoom"));
    ...
    if (sscanf(args.c_str(), "%*d %*s descriptio%c %n", &xtra, &len) >= 1) {
      ...
      if (newfl & 1) { // Can Open/Close
        ...
        door->SetSkill(prhash("Closeable"), 1);
        door->SetSkill(prhash("Locked"), 0);
        door->SetSkill(prhash("Lockable"), 1);
        door->SetSkill(prhash("Pickable"), 4);
        //	fprintf(stderr, CGRN "#%d Debug: %s door can open/close in
        //'%s'\n" CNRM,
        //		body->Skill(prhash("TBAScript")), dir.c_str(), line.c_str()
        //		);
      }
      if (newfl & 2) { // Closed
      ...
      if (newfl & 8) { // Pick-Proof
        door->SetSkill(prhash("Pickable"), 1000);
      ...
      tnum += 1000000;
      for (auto opt : options) {
        int onum = opt->Skill(prhash("TBARoom"));
        if (onum > 0 && onum == tnum) {
          toroom = opt;
          break;
        }
      ...
      door->SetSkill(prhash("Enterable"), 1);
      door->SetSkill(prhash("Open"), 1000);
      door->AddAct(act_t::SPECIAL_LINKED, odoor);
      odoor->SetSkill(prhash("Enterable"), 1);
      odoor->SetSkill(prhash("Open"), 1000);
      odoor->SetSkill(prhash("Invisible"), 1000);
      ...
      door->SetSkill(prhash("Lockable"), 1);
      door->SetSkill(prhash("Key"), 1000000 + tnum);
      if (door->Skill(prhash("Pickable")) < 1)
        door->SetSkill(prhash("Pickable"), 4);
      //      fprintf(stderr, CGRN "#%d Debug: %s door re-keyed (%d) in '%s'\n"
      //      CNRM,
      //	body->Skill(prhash("TBAScript")), dir.c_str(), tnum, line.c_str()
      //	);
    ...
    if (tbatype == 'o') {
      ...
      for (auto opt : options) {
        if (opt->Skill(prhash("TBAObject")) == valnum + 1000000) {
          item = new Object(*opt);
          break;
        }
      }
      ...
      auto options = src->Contents();
      for (auto opt : options) {
        if (opt->Skill(prhash("TBAMOB")) == valnum + 1000000) {
          item = new Object(*opt);
          break;
        }
      }
    ...
    if (params > 2) {
      ...
      //	body->Skill(prhash("TBAScript")), targ, line.c_str());
    ...
    if (strcmp("rfinger", where) == 0 || strcmp("1", where) == 0) {
      mask = item->Skill(prhash("Wearable on Right Finger"));
      loc = act_t::WEAR_RFINGER;
    } else if (strcmp("lfinger", where) == 0 || strcmp("2", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Finger"));
      loc = act_t::WEAR_LFINGER;
    } else if (strcmp("neck1", where) == 0 || strcmp("3", where) == 0) {
      mask = item->Skill(prhash("Wearable on Neck"));
      mask |= item->Skill(prhash("Wearable on Collar"));
      loc = act_t::WEAR_NECK;
      ...
      mask = item->Skill(prhash("Wearable on Chest"));
      mask &= item->Skill(prhash("Wearable on Back")); // Both
      loc = act_t::WEAR_CHEST;
    } else if (strcmp("head", where) == 0 || strcmp("6", where) == 0) {
      mask = item->Skill(prhash("Wearable on Head"));
      loc = act_t::WEAR_HEAD;
    } else if (strcmp("legs", where) == 0 || strcmp("7", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Leg"));
      mask |= item->Skill(prhash("Wearable on Right Leg"));
      loc = act_t::WEAR_LLEG;
    } else if (strcmp("feet", where) == 0 || strcmp("8", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Foot"));
      mask |= item->Skill(prhash("Wearable on Right Foot"));
      loc = act_t::WEAR_LFOOT;
    } else if (strcmp("hands", where) == 0 || strcmp("9", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Hand"));
      mask |= item->Skill(prhash("Wearable on Right Hand"));
      loc = act_t::WEAR_LHAND;
    } else if (strcmp("arms", where) == 0 || strcmp("10", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Arm"));
      mask |= item->Skill(prhash("Wearable on Right Arm"));
      loc = act_t::WEAR_LARM;
    } else if (strcmp("shield", where) == 0 || strcmp("11", where) == 0) {
      mask = item->Skill(prhash("Wearable on Shield"));
      loc = act_t::WEAR_SHIELD;
    } else if (strcmp("about", where) == 0 || strcmp("12", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Shoulder"));
      mask &= item->Skill(prhash("Wearable on Right Shoulder")); // Both
      loc = act_t::WEAR_LSHOULDER;
    } else if (strcmp("waist", where) == 0 || strcmp("13", where) == 0) {
      mask = item->Skill(prhash("Wearable on Waist"));
      loc = act_t::WEAR_WAIST;
    } else if (strcmp("rwrist", where) == 0 || strcmp("14", where) == 0) {
      mask = item->Skill(prhash("Wearable on Right Wrist"));
      loc = act_t::WEAR_RWRIST;
    } else if (strcmp("lwrist", where) == 0 || strcmp("15", where) == 0) {
      mask = item->Skill(prhash("Wearable on Left Wrist"));
...
uint32_t items[8] = {
    prhash("Food"),
    prhash("Hungry"), // Order is Most to Least Important
    prhash("Rest"),
    prhash("Tired"),
    prhash("Fun"),
    prhash("Bored"),
...
bool Mind::Think(int istick) {
  if (type == MIND_MOB) {
    if (body->Skill(prhash("Personality")) & 1) { // Group Mind
      //      body->TryCombine();	// I AM a group, after all.
      int qty = body->Skill(prhash("Quantity")), req = -1;
      if (qty < 1)
        qty = 1;
      ...
      if (req >= 0) {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          if (dir && dir->Skill(prhash("Open")) > 0) {
      ...
      //      if(body->Skill(prhash("Personality")) & 2) {		// Punk
      //	//body->BusyFor(500, "say Yo yo!");
      //	}
      //      else if(body->Skill(prhash("Personality")) & 4) {		// Normal
      //	//body->BusyFor(500, "say How do you do?");
      //	}
      //      else if(body->Skill(prhash("Personality")) & 8) {		// Rich
    ...
    if (body && body->Parent() && spos_s.size() > 0 && spos_s.back() < script.length()) {
      ...
      if (stype & 2) { // Random Triggers
        int chance = body->Skill(prhash("TBAScriptNArg")); // Percent Chance
        if (chance > 0) {
    ...
    if (body && body->Parent() && (body->Skill(prhash("TBAAction")) & 160) == 160 &&
        (!body->IsAct(act_t::FIGHT))) {
      auto others = body->PickObjects("everyone", LOC_NEARBY);
      for (auto other : others) {
        if ((!other->Skill(prhash("TBAAction"))) // FIXME: Other mobs?
            && body->Stun() < 6 // I'm not stunned
            && body->Phys() < 6 // I'm not injured
            && (!body->IsAct(act_t::SLEEP)) // I'm not asleep
            && (!body->IsAct(act_t::REST)) // I'm not resting
      ...
      if (istick == 1 && body->IsUsing(prhash("Perception"))) {
        body->BusyFor(500, "stop");
      } else if (
          istick == 0 // Triggered Only
          && (!body->IsUsing(prhash("Perception"))) // Not already searching
    ...
    // AGGRESSIVE and (!WIMPY) TBA Mobs
        ...
        body && body->Parent() && (body->Skill(prhash("TBAAction")) & 160) == 32 &&
        (!body->IsAct(act_t::FIGHT))) {
      auto others = body->PickObjects("everyone", LOC_NEARBY);
      for (auto other : others) {
        if ((!other->Skill(prhash("TBAAction"))) // FIXME: Other mobs?
    ...
    // HELPER TBA Mobs
    if (body && body->Parent() && (body->Skill(prhash("TBAAction")) & 4096) &&
        (!body->IsAct(act_t::FIGHT))) {
      auto others = body->PickObjects("everyone", LOC_NEARBY);
      for (auto other : others) {
        if ((!other->Skill(prhash("TBAAction"))) // FIXME: Other mobs?
            ...
            && other->ActTarg(act_t::FIGHT)->HasSkill(prhash("TBAAction"))
            //...against another MOB
        ) {
      ...
      if (!body->IsUsing(prhash("Perception"))) { // Don't let guard down!
        body->BusyFor(500, "search");
      } else if (
...
int Mind::TBACanWanderTo(Object* dest) const {
  if (dest->Skill(prhash("TBAZone")) == 999999) { // NO_MOBS TBA Zone
    return 0; // Don't Enter NO_MOBS Zone!
  } else if (body->Skill(prhash("TBAAction")) & 64) { // STAY_ZONE TBA MOBs
    if (dest->Skill(prhash("TBAZone")) != body->Parent()->Skill(prhash("TBAZone"))) {
      return 0; // Don't Leave Zone!
    }
  } else if (body->Skill(prhash("Swimming")) == 0) { // Can't Swim?
    if (dest->Skill(prhash("WaterDepth")) == 1) {
      return 0; // Can't swim!
    }
  } else if (!(body->Skill(prhash("TBAAffection")) & 64)) { // Can't Waterwalk?
    if (dest->Skill(prhash("WaterDepth")) >= 1) { // Need boat!
...
void Mind::Suspend(int msec) {
  // if(body && body->Skill(prhash("TBAScript")) >= 5034503 && body->Skill(prhash("TBAScript")) <=
  // 5034507)
  //  fprintf(stderr, CBLU "Suspended(%d): '%d'\n" CNRM, msec, body->Skill(prhash("TBAScript")));
