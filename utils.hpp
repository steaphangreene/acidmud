...
constexpr uint32_t crc32c(char const* str, int32_t len, uint32_t crc, int32_t pos) {
  return (pos == len)
      ? crc
      : crc32c(str, len, (crc >> 8) ^ crc32tab[(crc ^ ascii_tolower(str[pos])) & 0xFF], pos + 1);
}

constexpr uint32_t crc32c(char const* str, uint32_t crc, int32_t pos) {
  return (str[pos] == 0)
      ? crc
      : crc32c(str, (crc >> 8) ^ crc32tab[(crc ^ ascii_tolower(str[pos])) & 0xFF], pos + 1);
}

constexpr uint32_t crc32c(char const* str, int32_t len) {
  return crc32c(str, len, 0xFFFFFFFF, 0);
}
